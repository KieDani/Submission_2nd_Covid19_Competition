...
from training.loss import AbstractLoss
...
def evaluate(
    ...
    submission_sev_roc = -1

    f1_macro_sev = metrics.f1_score(all_sev_gt, pred_sev, average="macro")
    acc_sev = (all_sev_gt == pred_sev).float().mean()
    # same for inf
    f1_macro_inf = metrics.f1_score(all_inf_gt, pred_inf, average="macro")
    acc_inf = (all_inf_gt == pred_inf).float().mean()

    model.train()

    return {
        "acc_sev": acc_sev,
        "f1_macro_sev": f1_macro_sev,
        "acc_inf": acc_inf,
        "f1_macro_inf": f1_macro_inf,
        "auc_inf": inf_roc,
        "auc_sev": sev_roc,
