### Submission 05: segmentation (full) pretraining with cross validation; rotOrProb = 0.25
# FinalSeverityCV_balce-segmentationECCVFull_lrDecayUntil0_initModefull_rotProb0.25_AugModeall_cv_convnext_20220627-181820
...
import os
from pretraining.all.train_all import train_step_seg, train_step_mul1
...
inference_checkpoint = os.path.join(inference_checkpoint_dir, "sev5.pt")


def run_Sev(decayUntil=20, loss='balce', pretraining='imagenet', cv_enabeled=False, prenick='', init_mode='two_g',
            rotationProb=0, augmentations='all', max_epochs=60):
...
def run_segmia(config, args):
    ...
    if args.load == False:
        ...
        teacher, epoch = loadSequential(teacher, savepath=os.path.join(pretraining_DATA_PATH, 'saved_sup0/saved_sup0.pth'))
...
def train():
    ...
    prenick = 'SeverityPrediction/Submission05'
    cv_enabeled = True
    rotationProb = 0.25
    pretraining = 'segmentationECCVFull'
...
def pretrain():
    ...
    args = {
        'gpu': '0',
        'nick': 'SeverityPrediction/pretrainingSubmission05',
        'mode': 1,
        'load': False,
        'imsize': '256',
        'model': 'multinext'
...
def inference():
    args = {
        ...
        'output': 'output_sev_submission05.csv',
    }
    args = namedtuple("args", args.keys())(*args.values())
    logging.getLogger().setLevel(logging.INFO)
    ...
    checkpoint["config"]["modelconfig"]["pretrained_mode"] = "segmentationECCVFull"
    searchdir = Path(args.input)
    inputs = searchdir.glob("**/test_ct_scan*")
...
def main():
    #After the pretraining, you have to put the weights (checkpoint0.tar) in the correct directory!
    # pretrain()
    # train()
    inference()
